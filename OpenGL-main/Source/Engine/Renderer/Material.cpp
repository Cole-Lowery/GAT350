...
namespace neu {
	...
	void Material::Bind() {
		...
		if (shadowMap) {
			...
			shadowMap->Bind();
			program->SetUniform("u_shadowMap", 4);
	...
	void Material::UpdateGui() {
		if (ImGui::CollapsingHeader("Material", ImGuiTreeNodeFlags_DefaultOpen)) {
			...
			if (baseMap) {
				...
				Editor::GetDialogResource<Texture>(baseMap, "BaseMapDlg", "Open Teture", "Image (*.png; *.jpg; *.bmp;*jpeg;*tga){.png,.jpg,.bmp,.jpeg,.tga}");
			}
			...
			if (specularMap) {
				...
				Editor::GetDialogResource<Texture>(specularMap, "SpecularMapDlg", "Open Teture", "Image (*.png; *.jpg; *.bmp;*jpeg;*tga){.png,.jpg,.bmp,.jpeg,.tga}");
			}
			...
			if (emissiveMap) {
				...
				Editor::GetDialogResource<Texture>(emissiveMap, "emissiveMapDlg", "Open Teture", "Image (*.png; *.jpg; *.bmp;*jpeg;*tga){.png,.jpg,.bmp,.jpeg,.tga}");
			}
			...
			if (normalMap) {
				...
				Editor::GetDialogResource<Texture>(normalMap, "NormalMapDlg", "Open Teture", "Image (*.png; *.jpg; *.bmp;*jpeg;*tga){.png,.jpg,.bmp,.jpeg,.tga}");
			}
			...
			if (emissiveMap) ImGui::Text("Name: %s", emissiveMap->name.c_str());
